...
interface RequestDetailsViewProps {
  onClose: () => void;
  onSent?: (labels: string[]) => void;
}

const RequestDetailsView = ({ onClose, onSent }: RequestDetailsViewProps) => {
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-center gap-8 pt-4">
          ...
          <Button
            ...
            onClick={() => {
              toast.success(`${selected.length} detail request(s) sent to client`);
              onSent?.(selected.map(id => requestableDetails.find(d => d.id === id)?.label ?? id));
              onClose();
            }}
